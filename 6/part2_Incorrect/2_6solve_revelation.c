...
char nx=0, ny=0, nd = 0, ox = 0, oy = 0, od = 0;
int ops = 0;
    ...
    switch(map[x][y]){
        case(94):        //UP
            for(int i=x-1; i>=0; i--){
                ...
                if(override==1){
                    pos[i+1][y]=1;
                    if((i+1==24)&&(y==34)) printf("\n^\n");
                    map[i][y]='O';
                    if((i==23)&&(y==34)) ops=1;
                    if(traversal(i+1, y, 2, ops)==0){ 
                        obs[i][y]=1;
                        if(ops) printf("STARTS %d, %d", i+1, y);
                        //printf("%d,%d\n",i,y);
                    }
                    ops=0;
                    for(int l=0; l<150; l++){
                        for(int k=0; k<150; k++) memset(track[l][k], 0, 4);
                    }
                    map[i][y]='.';
                }
                map[i+1][y] = (override<2) ? '.' : 'X';
                if(i!=0) map[i][y]='^';
                else{
                    ...
                    if(override==2){
                        for(int m=0; m<size; m++){
                            for(int n=0; n<size; n++){
                                //if(map[m][n]=='X') map[m][n]='.';
                                if((map[m][n]!='.')&&(map[m][n]!='#')) map[m][n]='.';
                            }
                        }
                    }
                    return 1;
        ...
        case(62):           //RIGHT;
            for(int j=y+1; j<size; j++){
                ...
                if(override==1){
                    if((x==24)&&(j-1==34)) printf("\n>\n");
                    pos[x][j-1]=1;
                    map[x][j]='O';
                    if((x==23)&&(j==34)) ops=1;
                    if(traversal(x, j-1, 2, ops)==0){
                        obs[x][j]=1;
                        if(ops) printf("STARTS %d, %d", x, j-1);
                        //printf("%d,%d\n",x,j);
                    }
                    ops=0;
                    for(int l=0; l<150; l++){
