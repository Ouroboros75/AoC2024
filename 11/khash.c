...
void ht_init_data()
{
    int i;
    char buf[256];
    khint32_t x = 11;                                       //unsigned int
    printf("--- generating data... ");
    int_data = (unsigned*)calloc(data_size, sizeof(unsigned));      //unsigned defaults to unsigned int
    str_data = (char**)calloc(data_size, sizeof(char*));            //array of strings with [data_size] elements
    for (i = 0; i < data_size; ++i) {
        int_data[i] = (unsigned)(data_size * ((double)x / UINT_MAX) / 4) * 271828183u;      //??? scuffed hash function
        sprintf(buf, "%x", int_data[i]);        //write arbitrary data from int_data[i] to string "buf"
        str_data[i] = strdup(buf);              //strduplicate(buf) into str_data[i]    ???
